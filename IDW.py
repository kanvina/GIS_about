...
def get_z(data_arr,center_loction,p):
    ...
    wi_num=0
    wi_list= []
    for distance_point in distance_list:
    ...
    for i in range(len(wi_list)):
        ...
        z_i=data_arr[i,2]*wi_list[i]
        z_i_sum=z_i_sum+z_i

    return z_i_sum

def run_IDW(data_arr, row_size, column_size,cell_len,data_range,p):
    ...
    for index in range(row_size*column_size):
        ...
        z=get_z(data_arr,center_loction,p)
        data_IDW[row_i,column_i]=z
    return data_IDW

def main_IDW(path_point_xls,cell_len,p):
    ...
    data_IDW = run_IDW(data_arr, row_size, column_size, cell_len, data_range, p)
...
if __name__ =="__main__":
    ...
    data_IDW,data_range=main_IDW(path_point_xls, cell_len, p)
    print(data_IDW)
    print(data_range)

    pd.DataFrame(data_IDW).to_excel('data_out/data_IDW.xls',index=0)
